...
namespace TTERP.WebApi.Controllers
{
    ...
    public class InvoicesController : CustomBaseController
    {
        ...
        public async Task<IActionResult> GetList(int? orderId, bool? isActive, bool? isDeleted)
        {
            var result = await _mediator.Send(new GetInvoicesQuery(orderId, isActive, isDeleted));
